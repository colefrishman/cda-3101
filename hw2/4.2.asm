...
main:
    ...
    li $t3, 10              #stores constant to be compared against for the max
    li $t0, 2               #sets i to 2 for initial condition
    li $t4, 0               #sets last to 0 for initial condition
    li $t5, 1               #sets seclast to 1 for initial condition
    
Bumb:
    bgt	$t0, $t3, Exit	    #if i>10 then jump to exit

    add $t2, $t4, $zero     #gets value of previous bumble, stores in $t2
    sll $t2, $t2, 1         #multiplies $t2 by 2

#An overflow will occur for the largest values when using mult and mflo
    add $t1, $t5, $zero     #gets value of second-to-previous bumble, stores in $t1
    ...
    addi $t0, $t0, 1        #increments i by 1 for recursive loop
    add $t5, $t4, $zero     #stores last value into second-to-last for recursive loop
    add $t4, $t1, $zero     #stores current value into last for recursive loop
    addi $s3, $s3, 8        #increments memory locastion by 8
    j Bumb                  #jumps to label Bumb
